...
*/
class degreelistController extends Controller {
	private $PAGESIZE=10;
	public function getData($PageNum,QueryLogic $QueryLogic)
	{
		...
		if($UserID!=null)
            $QueryLogic->addCondition(new FieldCondition(itsap_degreeEntity::$ROLE_SYSTEMUSER_FID,$UserID));
		$degreeEnt=new itsap_degreeEntity($DBAccessor);
		$result['degree']=$degreeEnt;
		$allcount=$degreeEnt->FindAllCount($QueryLogic);
		$result['pagecount']=$this->getPageCount($allcount,$this->PAGESIZE);
		$QueryLogic->setLimit($this->getPageRowsLimit($PageNum,$this->PAGESIZE));
		$result['data']=$degreeEnt->FindAll($QueryLogic);
	...
	public function Search($PageNum,$title,$priority,$sortby,$isdesc)
	{
		...
		$sortByField=$degreeEnt->getTableField($sortby);
		if($sortByField!=null)
