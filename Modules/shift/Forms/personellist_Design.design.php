...
*/
class personellist_Design extends FormDesign {
	private $Data;
	/**
	 * @param mixed $Data
	 */
	 ...
	 * @return combobox
	 */
	public function getSemat_fid()
	{
		return $this->semat_fid;
	}
	/** @var textbox */
	private $shsh;
	/**
	...
	public function getBodyHTML($command=null)
	{
		...
		$Page->addElement($this->getPageTitlePart("فهرست " . $this->Data['personel']->getTableTitle() . " ها"));
		$LTable1=new Div();
		...
		$LTable1->addElement($this->getFieldRowCode($this->semat_fid,$this->getFieldCaption('semat_fid'),null,'',null));
		...
		$innerDiv[$i]->setClass("listitem");
			$url=new AppRooter('shift','personel');
		...
		$Page->addElement($this->getPaginationPart($this->Data['pagecount'],"shift","personellist"));
		$PageLink=new AppRooter('shift','personellist');
	...
	public function FillItems()
	{
		...
		foreach ($this->Data['eshteghal_fid'] as $item)
			...
			$this->semat_fid->addOption("", "مهم نیست");
		foreach ($this->Data['semat_fid'] as $item)
			$this->semat_fid->addOption($item->getID(), $item->getTitleField());
		...
		if (key_exists("personel", $this->Data)){
			...
			/******** semat_fid ********/
			$this->semat_fid->setSelectedValue($this->Data['personel']->getSemat_fid());
		...
		/******** semat_fid ********/
		$this->sortby->addOption($this->Data['personel']->getTableFieldID('semat_fid'),$this->getFieldCaption('semat_fid'));
		if(isset($_GET['semat_fid']))
			$this->semat_fid->setSelectedValue($_GET['semat_fid']);
	...
	public function __construct()
	{
		...
		/******* semat_fid *******/
		$this->semat_fid= new combobox("semat_fid");
		$this->semat_fid->setClass("form-control");
