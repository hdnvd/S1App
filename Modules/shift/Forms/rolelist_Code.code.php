...
use Modules\shift\Controllers\rolelistController;
...
*/
class rolelist_Code extends FormCode {
	private $searchForm='rolelist';
	...
	{
		$rolelistController=new rolelistController();
		$translator=new ModuleTranslator("shift");
		$translator->setLanguageName(CurrentLanguageManager::getCurrentLanguageName());
		try{
			$design=new rolelist_Design();
			...
			{
				$Result=$rolelistController->load($this->getHttpGETparameter('pn',-1));
	...
	public function search_Click()
	{
		...
		$rolelistController->setAdminMode($this->getAdminMode());
		$title=$this->getHttpGETparameter('title','');
		$sortby_ID=$this->getHttpGETparameter('sortby','');
		$isdesc_ID=$this->getHttpGETparameter('isdesc','');
		$Result=$rolelistController->Search($this->getHttpGETparameter('pn',-1),$title,$sortby_ID,$isdesc_ID);
		$design->setData($Result);
