...
*/
class visatypelistController extends Controller {
	private $PAGESIZE=10;
	public function getData($PageNum,QueryLogic $QueryLogic)
	{
		...
		if($UserID!=null)
            $QueryLogic->addCondition(new FieldCondition(iribfinance_visatypeEntity::$ROLE_SYSTEMUSER_FID,$UserID));
		$visatypeEnt=new iribfinance_visatypeEntity($DBAccessor);
		$result['visatype']=$visatypeEnt;
		$allcount=$visatypeEnt->FindAllCount($QueryLogic);
		$result['pagecount']=$this->getPageCount($allcount,$this->PAGESIZE);
		$QueryLogic->setLimit($this->getPageRowsLimit($PageNum,$this->PAGESIZE));
		$result['data']=$visatypeEnt->FindAll($QueryLogic);
	...
	public function Search($PageNum,$title,$sortby,$isdesc)
	{
		...
		$sortByField=$visatypeEnt->getTableField($sortby);
		if($sortByField!=null)
