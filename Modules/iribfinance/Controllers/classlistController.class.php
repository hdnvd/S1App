...
class classlistController extends Controller {
	...
	public function getData($PageNum,QueryLogic $QueryLogic)
	{
		...
		if($UserID!=null)
            $QueryLogic->addCondition(new FieldCondition(iribfinance_classEntity::$ROLE_SYSTEMUSER_FID,$UserID));
		$classEnt=new iribfinance_classEntity($DBAccessor);
		$result['class']=$classEnt;
		$allcount=$classEnt->FindAllCount($QueryLogic);
		$result['pagecount']=$this->getPageCount($allcount,$this->PAGESIZE);
		$QueryLogic->setLimit($this->getPageRowsLimit($PageNum,$this->PAGESIZE));
		$result['data']=$classEnt->FindAll($QueryLogic);
	...
	public function Search($PageNum,$title,$sortby,$isdesc)
	{
		...
		$sortByField=$classEnt->getTableField($sortby);
		if($sortByField!=null)
