...
class departmentlistController extends Controller {
	...
	public function getData($PageNum,QueryLogic $QueryLogic)
	{
		...
		if($UserID!=null)
            $QueryLogic->addCondition(new FieldCondition(iribfinance_departmentEntity::$ROLE_SYSTEMUSER_FID,$UserID));
		$departmentEnt=new iribfinance_departmentEntity($DBAccessor);
		$result['department']=$departmentEnt;
		$allcount=$departmentEnt->FindAllCount($QueryLogic);
		$result['pagecount']=$this->getPageCount($allcount,$this->PAGESIZE);
		$QueryLogic->setLimit($this->getPageRowsLimit($PageNum,$this->PAGESIZE));
		$result['data']=$departmentEnt->FindAll($QueryLogic);
	...
	public function Search($PageNum,$title,$region_fid,$sortby,$isdesc)
	{
		$DBAccessor=new dbaccess();
		$departmentEnt=new iribfinance_departmentEntity($DBAccessor);
		$q=new QueryLogic();
		...
		$q->addCondition(new FieldCondition("region_fid","%$region_fid%",LogicalOperator::LIKE));
		$sortByField=$departmentEnt->getTableField($sortby);
		if($sortByField!=null)
