...
class nationalitylistController extends Controller {
	...
	public function getData($PageNum,QueryLogic $QueryLogic)
	{
		...
		if($UserID!=null)
            $QueryLogic->addCondition(new FieldCondition(iribfinance_nationalityEntity::$ROLE_SYSTEMUSER_FID,$UserID));
		$nationalityEnt=new iribfinance_nationalityEntity($DBAccessor);
		$result['nationality']=$nationalityEnt;
		$allcount=$nationalityEnt->FindAllCount($QueryLogic);
		$result['pagecount']=$this->getPageCount($allcount,$this->PAGESIZE);
		$QueryLogic->setLimit($this->getPageRowsLimit($PageNum,$this->PAGESIZE));
		$result['data']=$nationalityEnt->FindAll($QueryLogic);
	...
	public function Search($PageNum,$title,$sortby,$isdesc)
	{
		...
		$sortByField=$nationalityEnt->getTableField($sortby);
		if($sortByField!=null)
