...
class regionlistController extends Controller {
	...
	public function getData($PageNum,QueryLogic $QueryLogic)
	{
		...
		if($UserID!=null)
            $QueryLogic->addCondition(new FieldCondition(iribfinance_regionEntity::$ROLE_SYSTEMUSER_FID,$UserID));
		$regionEnt=new iribfinance_regionEntity($DBAccessor);
		$result['region']=$regionEnt;
		$allcount=$regionEnt->FindAllCount($QueryLogic);
		$result['pagecount']=$this->getPageCount($allcount,$this->PAGESIZE);
		$QueryLogic->setLimit($this->getPageRowsLimit($PageNum,$this->PAGESIZE));
		$result['data']=$regionEnt->FindAll($QueryLogic);
	...
	public function Search($PageNum,$title,$sortby,$isdesc)
	{
		...
		$sortByField=$regionEnt->getTableField($sortby);
		if($sortByField!=null)
