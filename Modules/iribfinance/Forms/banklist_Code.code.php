...
use Modules\iribfinance\Controllers\banklistController;
...
*/
class banklist_Code extends FormCode {
	private $searchForm='banklist';
	...
	{
		$banklistController=new banklistController();
		$translator=new ModuleTranslator("iribfinance");
		$translator->setLanguageName(CurrentLanguageManager::getCurrentLanguageName());
		try{
			$design=new banklist_Design();
			...
			{
				$Result=$banklistController->load($this->getHttpGETparameter('pn',-1));
			if(isset($_GET['search']))
					$design=new banklistsearch_Design();
	...
	public function search_Click()
	{
		...
		$banklistController->setAdminMode($this->getAdminMode());
		$title=$this->getHttpGETparameter('title','');
		$sortby_ID=$this->getHttpGETparameter('sortby','');
		$isdesc_ID=$this->getHttpGETparameter('isdesc','');
		$Result=$banklistController->Search($this->getHttpGETparameter('pn',-1),$title,$sortby_ID,$isdesc_ID);
		$design->setData($Result);
