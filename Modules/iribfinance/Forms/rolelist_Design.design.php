...
*/
class rolelist_Design extends FormDesign {
	private $Data;
	/**
	 * @param mixed $Data
	 */
	...
	public function getBodyHTML($command=null)
	{
		...
		$Page->setId("iribfinance_rolelist");
		$Page->addElement($this->getPageTitlePart("فهرست " . $this->Data['role']->getTableTitle() . " ها"));
		$LTable1=new Div();
		...
		$innerDiv[$i]->setClass("listitem");
			$url=new AppRooter('iribfinance','role');
		...
		$Page->addElement($this->getPaginationPart($this->Data['pagecount'],"iribfinance","rolelist"));
		$PageLink=new AppRooter('iribfinance','rolelist');
	...
	{
		if (key_exists("role", $this->Data)){

			/******** title ********/
			$this->title->setValue($this->Data['role']->getTitle());
			$this->setFieldCaption('title',$this->Data['role']->getFieldInfo('title')->getTitle());
		...
		/******** title ********/
		$this->sortby->addOption($this->Data['role']->getTableFieldID('title'),$this->getFieldCaption('title'));
		if(isset($_GET['title']))
