...
use Modules\iribfinance\Controllers\nationalitylistController;
...
*/
class nationalitylist_Code extends FormCode {
	private $searchForm='nationalitylist';
	...
	{
		$nationalitylistController=new nationalitylistController();
		$translator=new ModuleTranslator("iribfinance");
		$translator->setLanguageName(CurrentLanguageManager::getCurrentLanguageName());
		try{
			$design=new nationalitylist_Design();
			...
			{
				$Result=$nationalitylistController->load($this->getHttpGETparameter('pn',-1));
			if(isset($_GET['search']))
					$design=new nationalitylistsearch_Design();
	...
	public function __construct($namespace)
	{
		...
		$this->setTitle("Nationality List");
	}
	public function search_Click()
	{
		...
		$nationalitylistController->setAdminMode($this->getAdminMode());
		$title=$this->getHttpGETparameter('title','');
		$sortby_ID=$this->getHttpGETparameter('sortby','');
		$isdesc_ID=$this->getHttpGETparameter('isdesc','');
		$Result=$nationalitylistController->Search($this->getHttpGETparameter('pn',-1),$title,$sortby_ID,$isdesc_ID);
		$design->setData($Result);
