...
*/
class manageprogramestimations_Design extends FormDesign {
	private $Data;
	/**
	 * @param mixed $Data
	 */
    ...
    public function setAdminMode($adminMode)
    {
        ...
        $this->itemViewPage = 'programestimation';
        if($adminMode==true)
        {
            $this->itemPage = 'manageprogramestimation';
            $this->listPage = 'manageprogramestimations';
        }
        else
        {
            $this->itemPage = 'manageuserprogramestimation';
            $this->listPage = 'manageuserprogramestimations';
        }
    }
	...
	public function getBodyHTML($command=null)
	{
		...
		$Page->setId("iribfinance_manageprogramestimations");
		$Page->addElement($this->getPageTitlePart("مدیریت " . $this->Data['programestimation']->getTableTitle() . " ها"));
		...
		$lnkAdd->setId('addprogramestimationlink');
		...
		$lnkSearch->setId('searchprogramestimationlink');
		...
		for($i=0;$i<count($this->Data['data']);$i++){
            ...
            $ManageEmployeesURL=new AppRooter('iribfinance','manageprogramestimationemployees');
            $ManageEmployeesURL->addParameter(new UrlParameter('estimationid',$this->Data['data'][$i]->getID()));
            $lbManageEmployees[$i]=new Lable('مدیریت عوامل');
            $lnkManageEmployees[$i]=new link($ManageEmployeesURL->getAbsoluteURL(),$lbManageEmployees[$i]);
            $lnkManageEmployees[$i]->setGlyphiconClass('glyphicon glyphicon-eye-open');
            $lnkManageEmployees[$i]->setClass('btn btn-primary');
			...
			$operationDiv[$i]->addElement($lnkManageEmployees[$i]);
		...
		$Page->addElement($this->getPaginationPart($this->Data['pagecount'],"iribfinance",$this->listPage));
