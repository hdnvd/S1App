...
*/
class manageprogramestimationemployees_Design extends FormDesign {
	private $Data;
	/**
	 * @param mixed $Data
	 */
    ...
    public function setAdminMode($adminMode)
    {
        ...
        $this->itemViewPage = 'programestimationemployee';
        if($adminMode==true)
        {
            $this->itemPage = 'manageprogramestimationemployee';
            $this->listPage = 'manageprogramestimationemployees';
        }
        else
        {
            $this->itemPage = 'manageuserprogramestimationemployee';
            $this->listPage = 'manageuserprogramestimationemployees';
        }
    }
	...
	public function getBodyHTML($command=null)
	{
		...
		$Page->setId("iribfinance_manageprogramestimationemployees");
		$Page->addElement($this->getPageTitlePart("مدیریت " . $this->Data['programestimationemployee']->getTableTitle() . " ها"));
		$addUrl=new AppRooter('iribfinance',$this->itemPage);
        $addUrl->addParameter(new UrlParameter('estimationid',$_GET['estimationid']));
		...
		$lnkAdd->setId('addprogramestimationemployeelink');
		$Page->addElement($lnkAdd);
		$SearchUrl=new AppRooter('iribfinance',$this->listPage);
		...
		$lnkSearch->setId('searchprogramestimationemployeelink');
		...
		$LTable1->setLastElementClass("listtitle");
        $LTable1->addElement(new Lable('فعالیت'));
        $LTable1->setLastElementClass("listtitle");
		$LTable1->addElement(new Lable('عملیات'));
		$LTable1->setLastElementClass("listtitle");
		for($i=0;$i<count($this->Data['data']);$i++){
			$url=new AppRooter('iribfinance',$this->itemPage);
			$url->addParameter(new UrlParameter('id',$this->Data['data'][$i]->getID()));
            $url->addParameter(new UrlParameter('estimationid',$_GET['estimationid']));
			$Title=$this->Data['employee'][$i]->getName() . " " . $this->Data['employee'][$i]->getFamily();
            $Activity=$this->Data['activity'][$i]->getTitle();
			if($Title=="")
				$Title='- بدون عنوان -';
			$lbTit[$i]=new Lable($Title);
            $lbAct[$i]=new Lable($Activity);
			$liTit[$i]=new link($url->getAbsoluteURL(),$lbTit[$i]);
			$ViewURL=new AppRooter('iribfinance',$this->itemViewPage);
			...
			$delurl=new AppRooter('iribfinance',$this->listPage);
		...
		$Page->addElement($this->getPaginationPart($this->Data['pagecount'],"iribfinance",$this->listPage,[new UrlParameter('estimationid',$_GET['estimationid'])]));
