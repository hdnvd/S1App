...
*/
class paycenterlist_Design extends FormDesign {
	private $Data;
	/**
	 * @param mixed $Data
	 */
	...
	public function getBodyHTML($command=null)
	{
		...
		$Page->setId("iribfinance_paycenterlist");
		$Page->addElement($this->getPageTitlePart("فهرست " . $this->Data['paycenter']->getTableTitle() . " ها"));
		$LTable1=new Div();
		$LTable1->setClass("searchtable");
		$LTable1->addElement($this->getFieldRowCode($this->title,$this->getFieldCaption('title'),null,'',null));
		$LTable1->addElement($this->getFieldRowCode($this->chapter,$this->getFieldCaption('chapter'),null,'',null));
		$LTable1->addElement($this->getFieldRowCode($this->accountingcode,$this->getFieldCaption('accountingcode'),null,'',null));
		...
		$innerDiv[$i]->setClass("listitem");
			$url=new AppRooter('iribfinance','paycenter');
		...
		$Page->addElement($this->getPaginationPart($this->Data['pagecount'],"iribfinance","paycenterlist"));
		$PageLink=new AppRooter('iribfinance','paycenterlist');
	...
	public function FillItems()
	{
		...
		/******** title ********/
		$this->sortby->addOption($this->Data['paycenter']->getTableFieldID('title'),$this->getFieldCaption('title'));
		if(isset($_GET['title']))
			$this->title->setValue($_GET['title']);

		/******** chapter ********/
		$this->sortby->addOption($this->Data['paycenter']->getTableFieldID('chapter'),$this->getFieldCaption('chapter'));
		if(isset($_GET['chapter']))
			$this->chapter->setValue($_GET['chapter']);

		/******** accountingcode ********/
		$this->sortby->addOption($this->Data['paycenter']->getTableFieldID('accountingcode'),$this->getFieldCaption('accountingcode'));
		if(isset($_GET['accountingcode']))
			$this->accountingcode->setValue($_GET['accountingcode']);
