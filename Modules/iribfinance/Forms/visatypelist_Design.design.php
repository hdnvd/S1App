...
*/
class visatypelist_Design extends FormDesign {
	private $Data;
	/**
	 * @param mixed $Data
	 */
	...
	public function getBodyHTML($command=null)
	{
		...
		$Page->setId("iribfinance_visatypelist");
		$Page->addElement($this->getPageTitlePart("فهرست " . $this->Data['visatype']->getTableTitle() . " ها"));
		$LTable1=new Div();
		...
		$innerDiv[$i]->setClass("listitem");
			$url=new AppRooter('iribfinance','visatype');
		...
		$Page->addElement($this->getPaginationPart($this->Data['pagecount'],"iribfinance","visatypelist"));
		$PageLink=new AppRooter('iribfinance','visatypelist');
	...
	{
		if (key_exists("visatype", $this->Data)){

			/******** title ********/
			$this->title->setValue($this->Data['visatype']->getTitle());
			$this->setFieldCaption('title',$this->Data['visatype']->getFieldInfo('title')->getTitle());
		...
		/******** title ********/
		$this->sortby->addOption($this->Data['visatype']->getTableFieldID('title'),$this->getFieldCaption('title'));
		if(isset($_GET['title']))
