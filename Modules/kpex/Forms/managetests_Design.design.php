...
PlaySound('boom');
   ...
   <source src=\"".DEFAULT_PUBLICURL."content/files/1.mp3\" type=\"audio/wav\">
        ...
        $makeMajuroCSV=new AppRooter('kpex',$this->listPage);
        $makeMajuroCSV->addParameter(new URLParameter('majuro',null));
        $LblmakeMajuroCSV=new Lable('Make Majuro CSV' );
        $lnkmakeMajuroCSV=new link($makeMajuroCSV->getAbsoluteURL(),$LblmakeMajuroCSV);
        $lnkmakeMajuroCSV->setClass('linkbutton btn btn-primary');
        $lnkmakeMajuroCSV->setGlyphiconClass('glyphicon glyphicon-plus');
        $lnkmakeMajuroCSV->setId('addtestlink');
        $Page->addElement($lnkmakeMajuroCSV);

        $makeSemevalCSV=new AppRooter('kpex',$this->listPage);
        $makeSemevalCSV->addParameter(new URLParameter('semeval',null));
