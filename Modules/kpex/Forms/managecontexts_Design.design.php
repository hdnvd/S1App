...
*/
class managecontexts_Design extends FormDesign {
	private $Data;
	/**
	 * @param mixed $Data
	 */
    ...
    public function setAdminMode($adminMode)
    {
        ...
        $this->itemViewPage = 'context';
        if($adminMode==true)
        {
            $this->itemPage = 'managecontext';
            $this->listPage = 'managecontexts';
        }
        else
        {
            $this->itemPage = 'manageusercontext';
            $this->listPage = 'manageusercontexts';
        }
    }
	...
	public function getBodyHTML($command=null)
	{
		...
		$Page->setId("kpex_managecontexts");
		$Page->addElement($this->getPageTitlePart(" " . $this->Data['context']->getTableTitle() . " ها"));
		$addUrl=new AppRooter('kpex',$this->itemPage);
		$LblAdd=new Lable('ثبت ' . $this->Data['context']->getTableTitle() . ' جدید');
		...
		$lnkSearch->setId('searchcontextlink');
		...
		for($i=0;$i<count($this->Data['data']);$i++){
			...
			$ViewURL=new AppRooter('kpex',$this->itemViewPage);
