...
class transactionlist_Design extends FormDesign {
	...
	public function getBodyHTML($command=null)
	{
		...
		foreach ($this->Data['chapter_fid'] as $item)
			$this->chapter_fid->addOption($item->getID(), $item->getTitle());
		if (key_exists("transaction", $this->Data))
		...
		$PageTitlePart->addElement(new Lable("فهرست تراکنش های سیستم"));
        ...
        $LTable1->addElement(new Lable(' مبلغ به ریال'));
        $LTable1->setLastElementClass("listtitle");
        ...
        $LTable1->setLastElementClass("listtitle");
        $LTable1->addElement(new Lable('پرداخت کننده'));
        $LTable1->setLastElementClass("listtitle");
        ...
        for($i=0;$i<count($this->Data['data']);$i++){
            ...
            $LTable1->addElement(new Lable($this->Data['data'][$i]->getAmount()));
            $LTable1->setLastElementClass("listcontent");
            ...
            $LTable1->setLastElementClass("listcontent");
            $name=$this->Data['bankpayment'][$i]->getName() . " " . $this->Data['bankpayment'][$i]->getFamily();

            if($this->Data['data'][$i]->getRole_systemuser_fid()>0 && trim($name)=="")
                $name="کاربر شماره ".$this->Data['data'][$i]->getRole_systemuser_fid();
