...
*/
class managefiletransactions_Design extends FormDesign {
	private $Data;
	/**
	 * @param mixed $Data
	 */
    ...
    public function setAdminMode($adminMode)
    {
        ...
        $this->itemViewPage = 'filetransaction';
        if($adminMode==true)
        {
            $this->itemPage = 'managefiletransaction';
            $this->listPage = 'managefiletransactions';
        }
        else
        {
            $this->itemPage = 'manageuserfiletransaction';
            $this->listPage = 'manageuserfiletransactions';
        }
    }
	...
	public function getBodyHTML($command=null)
	{
		...
		$Page->setId("fileshop_managefiletransactions");
		$Page->addElement($this->getPageTitlePart("مدیریت " . $this->Data['filetransaction']->getTableTitle() . " ها"));
		$addUrl=new AppRooter('fileshop',$this->itemPage);
		...
		$lnkAdd->setId('addfiletransactionlink');
		$Page->addElement($lnkAdd);
		$SearchUrl=new AppRooter('fileshop',$this->listPage);
		...
		$lnkSearch->setId('searchfiletransactionlink');
		...
		for($i=0;$i<count($this->Data['data']);$i++){
			$url=new AppRooter('fileshop',$this->itemPage);
			...
			$ViewURL=new AppRooter('fileshop',$this->itemViewPage);
			...
			$delurl=new AppRooter('fileshop',$this->listPage);
		...
		$Page->addElement($this->getPaginationPart($this->Data['pagecount'],"fileshop",$this->listPage));
