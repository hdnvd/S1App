...
*/
class managesystemtasks_Design extends FormDesign {
	private $Data;
	/**
	 * @param mixed $Data
	 */
    ...
    public function setAdminMode($adminMode)
    {
        ...
        $this->itemViewPage = 'systemtask';
        if($adminMode==true)
        {
            $this->itemPage = 'managesystemtask';
            $this->listPage = 'managesystemtasks';
        }
        else
        {
            $this->itemPage = 'manageusersystemtask';
            $this->listPage = 'manageusersystemtasks';
        }
    }
	...
	public function getBodyHTML($command=null)
	{
		...
		$Page->setId("users_managesystemtasks");
		$Page->addElement($this->getPageTitlePart("مدیریت " . $this->Data['systemtask']->getTableTitle() . " ها"));
		...
		$lnkAdd->setId('addsystemtasklink');
		...
		$lnkSearch->setId('searchsystemtasklink');
		...
		$LTable1->setLastElementClass("listtitle");
        $LTable1->addElement(new Lable('صفحه'));
        $LTable1->setLastElementClass("listtitle");
		...
		for($i=0;$i<count($this->Data['data']);$i++){
			...
			$Title=$this->Data['data'][$i]->getModule();
            $PageName=$this->Data['data'][$i]->getPage();
			if($Title=="")
				$Title='- بدون عنوان -';
			$lbTit[$i]=new Lable($PageName);
