...
*/
class manageusercourses_Design extends FormDesign {
	private $Data;
	/**
	 * @param mixed $Data
	 */
    ...
    public function setAdminMode($adminMode)
    {
        ...
        $this->itemViewPage = 'usercourse';
        if($adminMode==true)
        {
            $this->itemPage = 'manageusercourse';
            $this->listPage = 'manageusercourses';
        }
        else
        {
            $this->itemPage = 'manageuserusercourse';
            $this->listPage = 'manageuserusercourses';
        }
    }
	...
	public function getBodyHTML($command=null)
	{
		...
		$Page->setId("onlineclass_manageusercourses");
		$Page->addElement($this->getPageTitlePart("مدیریت " . $this->Data['usercourse']->getTableTitle() . " ها"));
		$addUrl=new AppRooter('onlineclass',$this->itemPage);
		...
		$lnkAdd->setId('addusercourselink');
		$Page->addElement($lnkAdd);
		$SearchUrl=new AppRooter('onlineclass',$this->listPage);
		...
		$lnkSearch->setId('searchusercourselink');
		...
		for($i=0;$i<count($this->Data['data']);$i++){
			$url=new AppRooter('onlineclass',$this->itemPage);
			...
			$ViewURL=new AppRooter('onlineclass',$this->itemViewPage);
			...
			$delurl=new AppRooter('onlineclass',$this->listPage);
		...
		$Page->addElement($this->getPaginationPart($this->Data['pagecount'],"onlineclass",$this->listPage));
