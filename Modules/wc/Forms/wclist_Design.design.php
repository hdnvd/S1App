...
*/
class wclist_Design extends FormDesign {
	private $Data;
	/**
	 * @param mixed $Data
	 */
	...
	public function getJSON()
    {
        ...
        for ($i = 0; $i < count($this->Data['data']); $i++) {
            $arr['locations'][$i]['latitude']=$this->Data['data'][$i]->getLatitude();
            $arr['locations'][$i]['longitude']=$this->Data['data'][$i]->getLongitude();
            $arr['locations'][$i]['placetitle']=$this->Data['data'][$i]->getPlacetitle();
        }
    ...
    public function getBodyHTML($command=null)
	{
		...
		$Page->addElement($this->getPageTitlePart("فهرست " . $this->Data['wc']->getTableTitle() . " ها"));
		$LTable1=new Div();
		...
		$Page->addElement($this->getPaginationPart($this->Data['pagecount'],"wc","wclist"));
		$PageLink=new AppRooter('wc','wclist');
