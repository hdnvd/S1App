...
*/
class managewcs_Design extends FormDesign {
	private $Data;
	/**
	 * @param mixed $Data
	 */
    ...
    public function setAdminMode($adminMode)
    {
        ...
        $this->itemViewPage = 'wc';
        if($adminMode==true)
        {
            $this->itemPage = 'managewc';
            $this->listPage = 'managewcs';
        }
        else
        {
            $this->itemPage = 'manageuserwc';
            $this->listPage = 'manageuserwcs';
        }
    }
	...
	public function getBodyHTML($command=null)
	{
		...
		$Page->setId("wc_managewcs");
		$Page->addElement($this->getPageTitlePart("مدیریت " . $this->Data['wc']->getTableTitle() . " ها"));
		$addUrl=new AppRooter('wc',$this->itemPage);
		...
		$lnkAdd->setId('addwclink');
		$Page->addElement($lnkAdd);
		$SearchUrl=new AppRooter('wc',$this->listPage);
		...
		for($i=0;$i<count($this->Data['data']);$i++){
			$url=new AppRooter('wc',$this->itemPage);
			...
			$ViewURL=new AppRooter('wc',$this->itemViewPage);
			...
			$delurl=new AppRooter('wc',$this->listPage);
		...
		$Page->addElement($this->getPaginationPart($this->Data['pagecount'],"wc",$this->listPage));
