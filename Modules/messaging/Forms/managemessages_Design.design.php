...
*/
class managemessages_Design extends FormDesign {
	private $Data;
	/**
	 * @param mixed $Data
	 */
    ...
    public function setAdminMode($adminMode)
    {
        ...
        $this->itemViewPage = 'message';
        if($adminMode==true)
        {
            $this->itemPage = 'managemessage';
            $this->listPage = 'managemessages';
        }
        else
        {
            $this->itemPage = 'manageusermessage';
            $this->listPage = 'manageusermessages';
        }
    }
	...
	public function getBodyHTML($command=null)
	{
		...
		$Page->setId("messaging_managemessages");
		$Page->addElement($this->getPageTitlePart("مدیریت " . $this->Data['message']->getTableTitle() . " ها"));
		$addUrl=new AppRooter('messaging',$this->itemPage);
		...
		$lnkAdd->setId('addmessagelink');
		$Page->addElement($lnkAdd);
		$SearchUrl=new AppRooter('messaging',$this->listPage);
		...
		$lnkSearch->setId('searchmessagelink');
		...
		for($i=0;$i<count($this->Data['data']);$i++){
			$url=new AppRooter('messaging',$this->itemPage);
			...
			$ViewURL=new AppRooter('messaging',$this->itemViewPage);
			...
			$delurl=new AppRooter('messaging',$this->listPage);
		...
		$Page->addElement($this->getPaginationPart($this->Data['pagecount'],"messaging",$this->listPage));
