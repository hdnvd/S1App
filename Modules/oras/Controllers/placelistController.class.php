...
class placelistController extends Controller {
	...
	public function getData($PageNum,QueryLogic $QueryLogic)
	{
		...
		if($UserID!=null)
            $QueryLogic->addCondition(new FieldCondition(oras_placeEntity::$ROLE_SYSTEMUSER_FID,$UserID));
		$placeEnt=new oras_placeEntity($DBAccessor);
		$result['place']=$placeEnt;
		$allcount=$placeEnt->FindAllCount($QueryLogic);
		$result['pagecount']=$this->getPageCount($allcount,$this->PAGESIZE);
		$QueryLogic->setLimit($this->getPageRowsLimit($PageNum,$this->PAGESIZE));
		$result['data']=$placeEnt->FindAll($QueryLogic);
	...
	public function Search($PageNum,$title,$sortby,$isdesc)
	{
		...
		$sortByField=$placeEnt->getTableField($sortby);
		if($sortByField!=null)
