...
*/
class managedoctorfiles_Design extends FormDesign {
	private $Data;
	/**
	 * @param mixed $Data
	 */
    ...
    public function setAdminMode($adminMode)
    {
        ...
        $this->itemViewPage = 'doctorfile';
        if($adminMode==true)
        {
            $this->itemPage = 'managedoctorfile';
            $this->listPage = 'managedoctorfiles';
        }
        else
        {
            $this->itemPage = 'manageuserdoctorfile';
            $this->listPage = 'manageuserdoctorfiles';
        }
    }
	...
	public function getBodyHTML($command=null)
	{
		...
		$Page->setId("ocms_managedoctorfiles");
		$Page->addElement($this->getPageTitlePart("مدیریت " . $this->Data['doctorfile']->getTableTitle() . " ها"));
		$addUrl=new AppRooter('ocms',$this->itemPage);
		$LblAdd=new Lable('آپلود فایل جدید');
		...
		$lnkAdd->setId('adddoctorfilelink');
		...
		$lnkSearch->setId('searchdoctorfilelink');
//		$Page->addElement($lnkSearch);
		...
		for($i=0;$i<count($this->Data['data']);$i++){
			...
			$lnkView[$i]=new link(DEFAULT_PUBLICURL . $this->Data['data'][$i]->getFile_flu(),$lbView[$i]);
