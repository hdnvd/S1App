...
*/
class categorylist_Design extends FormDesign {
	private $Data;
	/**
	 * @param mixed $Data
	 */
	...
	public function getBodyHTML($command=null)
	{
		...
		$Page->setId("common_categorylist");
		$Page->addElement($this->getPageTitlePart("فهرست " . $this->Data['category']->getTableTitle() . " ها"));
		$LTable1=new Div();
		...
		$LTable1->addElement($this->getFieldRowCode($this->category_fid,$this->getFieldCaption('category_fid'),null,'',null));
		...
		$innerDiv[$i]->setClass("listitem");
			$url=new AppRooter('common','category');
		...
		$Page->addElement($this->getPaginationPart($this->Data['pagecount'],"common","categorylist"));
		$PageLink=new AppRooter('common','categorylist');
	...
	{
			$this->category_fid->addOption("", "مهم نیست");
		...
		/******** title ********/
		$this->sortby->addOption($this->Data['category']->getTableFieldID('title'),$this->getFieldCaption('title'));
		if(isset($_GET['title']))
			$this->title->setValue($_GET['title']);

		/******** latintitle ********/
		$this->sortby->addOption($this->Data['category']->getTableFieldID('latintitle'),$this->getFieldCaption('latintitle'));
		if(isset($_GET['latintitle']))
			$this->latintitle->setValue($_GET['latintitle']);

		/******** category_fid ********/
		$this->sortby->addOption($this->Data['category']->getTableFieldID('category_fid'),$this->getFieldCaption('category_fid'));
		if(isset($_GET['category_fid']))
			$this->category_fid->setSelectedValue($_GET['category_fid']);
