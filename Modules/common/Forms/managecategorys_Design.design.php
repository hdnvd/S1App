...
*/
class managecategorys_Design extends FormDesign {
	private $Data;
	/**
	 * @param mixed $Data
	 */
    ...
    public function setAdminMode($adminMode)
    {
        ...
        $this->itemViewPage = 'category';
        if($adminMode==true)
        {
            $this->itemPage = 'managecategory';
            $this->listPage = 'managecategorys';
        }
        else
        {
            $this->itemPage = 'manageusercategory';
            $this->listPage = 'manageusercategorys';
        }
    }
	...
	public function getBodyHTML($command=null)
	{
		...
		$Page->setId("common_managecategorys");
		$Page->addElement($this->getPageTitlePart("مدیریت " . $this->Data['category']->getTableTitle() . " ها"));
		$addUrl=new AppRooter('common',$this->itemPage);
		...
		$lnkAdd->setId('addcategorylink');
		$Page->addElement($lnkAdd);
		$SearchUrl=new AppRooter('common',$this->listPage);
		...
		$lnkSearch->setId('searchcategorylink');
		...
		for($i=0;$i<count($this->Data['data']);$i++){
			$url=new AppRooter('common',$this->itemPage);
			...
			$ViewURL=new AppRooter('common',$this->itemViewPage);
			...
			$delurl=new AppRooter('common',$this->listPage);
		...
		$Page->addElement($this->getPaginationPart($this->Data['pagecount'],"common",$this->listPage));
