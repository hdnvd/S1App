...
*/
class manageproducts_Design extends FormDesign {
	private $Data;
	/**
	 * @param mixed $Data
	 */
    ...
    public function setAdminMode($adminMode)
    {
        ...
        $this->itemViewPage = 'product';
        if($adminMode==true)
        {
            $this->itemPage = 'manageproduct';
            $this->listPage = 'manageproducts';
        }
        else
        {
            $this->itemPage = 'manageuserproduct';
            $this->listPage = 'manageuserproducts';
        }
    }
	...
	public function getBodyHTML($command=null)
	{
		...
		$Page->setId("eshop_manageproducts");
		$Page->addElement($this->getPageTitlePart("مدیریت " . $this->Data['product']->getTableTitle() . " ها"));
		$addUrl=new AppRooter('eshop',$this->itemPage);
		...
		$lnkAdd->setId('addproductlink');
		$Page->addElement($lnkAdd);
		$SearchUrl=new AppRooter('eshop',$this->listPage);
		...
		$lnkSearch->setId('searchproductlink');
		...
		for($i=0;$i<count($this->Data['data']);$i++){
			$url=new AppRooter('eshop',$this->itemPage);
			...
			$ViewURL=new AppRooter('eshop',$this->itemViewPage);
			...
			$delurl=new AppRooter('eshop',$this->listPage);
		...
		$Page->addElement($this->getPaginationPart($this->Data['pagecount'],"eshop",$this->listPage));
